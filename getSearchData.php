...
//echo $_REQUEST["id"];
    ...
    if($_REQUEST["searchterm"]!="")
        $condition=" AND products_name LIKE '%".$_REQUEST["searchterm"]."%'";
    //Get all rows except already displayed
    $sql="SELECT * FROM products WHERE products_status='1'". $condition. "  ORDER BY products_id DESC";
    $result = $dbh->prepare($sql);
    $result->execute();
    //$totalproducts = $result->fetchAll(PDO::FETCH_OBJ);
    $allNumRows=$result->rowCount();
    
    $sql="SELECT * FROM products WHERE products_status='1'" . $condition." ORDER BY products_id DESC LIMIT ".$showLimit;
    $result = $dbh->prepare($sql);
    $result->execute();
            ...
            <div class="col-xs-3 col-sm-3 col-md-3">
                ...
                <div id="spanqty<?php echo $products[$i]->products_id; ?>" <?php if($incart==0){ ?> style="display:none;" <?php } ?>>
                    <div class="input-group">
                        ...
                        id="qty<?php echo $products[$i]->products_id; ?>" value="<?php echo $cartquantity; ?>">
                        <div class="input-group-btn">
